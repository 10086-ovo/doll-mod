package com.example.doll.screen;

import com.example.doll.DollModConstants;
import com.example.doll.mode.DollMode;
import com.example.doll.network.DollClientNetworking;
 ...
 * 收集玩家所有存活人偶并通过 {@link com.example.doll.network.payload.OpenDollControlPanelPayload}
...
public class DollControlScreen extends Screen {
	...
	private void renderList(GuiGraphicsExtractor g) {
		Component title = Component.translatable("container." + DollModConstants.MOD_ID + ".control_panel");
		g.centeredText(this.font, title, leftPos + PANEL_W / 2, topPos + 12, COLOR_NAME);
		...
		for (int i = 0; i < visible; i++) {
			...
			g.text(this.font, status, leftPos + 120, rowY + 8, statusColor, true);

			// 离线人偶：显示最后坐标 / 在线人偶：显示召回按钮
			if (d.entityId() < 0) {
				// 离线人偶 → 显示坐标
				String coords = d.lastX() + ", " + d.lastY() + ", " + d.lastZ();
				...
				// 在线人偶 → 召回按钮（末影珍珠图标）
				int recallX = leftPos + PANEL_W - 52;
			...
			if (d.inSameDimension() && d.entityId() >= 0) {
				...
				g.text(this.font, dist, leftPos + PANEL_W - 8 - this.font.width(dist), rowY + 8, COLOR_STATUS, true);
			}
		}
