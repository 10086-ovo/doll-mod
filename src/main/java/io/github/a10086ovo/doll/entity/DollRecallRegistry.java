...
public final class DollRecallRegistry {
	...
	private static final Map<UUID, DollLocation> LOCATIONS = new HashMap<>();
	...
	public static void record(UUID uuid, ResourceKey<Level> dimension, BlockPos pos, UUID ownerUuid) {
		if (uuid != null) {
			LOCATIONS.put(uuid, new DollLocation(dimension, pos.immutable(), ownerUuid));
		}
	}
	...
	/** 获取所有登记位置的不可变视图（用于控制面板展示离线人偶）。调用方无法修改内部状态。 */
